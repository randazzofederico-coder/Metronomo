...
float BPMDetect::getBpm()
{
    ...
    // find peak position
    detectedPeakPos = peakFinder.detectPeak(data, windowStart, windowLen);
    ...
    assert(decimateBy != 0);
    if (detectedPeakPos < 1e-9) return 0.0; // detection failed.

    _SaveDebugBeatPos("soundtouch-detected-beats.txt", beats);

    // calculate BPM
    float bpm = (float)(coeff / detectedPeakPos);
...
int BPMDetect::getBeats(float *outPos, float *values, int max_num)
{
    int num = (int)beats.size();
    if ((!outPos) || (!values)) return num;    // outPos or values nullptr, return just size

    for (int i = 0; (i < num) && (i < max_num); i++)
    {
        outPos[i] = beats[i].pos;
